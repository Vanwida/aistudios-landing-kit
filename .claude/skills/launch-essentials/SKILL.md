...
```
Put the endpoint/key in `.env` (`PUBLIC_FORM_ENDPOINT`, `PUBLIC_FORM_KEY`) and in Vercel env vars. Ask the designer only: "Which email should receive the messages?" Then set up the service with that email (Web3Forms: enter the email on their site → key arrives by email → the designer pastes it into `.env`, not in chat).
